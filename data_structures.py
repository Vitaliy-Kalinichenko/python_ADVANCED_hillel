...
# Напишите решение функции def (A), которое для массива A из N целых чисел возвращает наименьшее положительное целое число (больше 0), которое не встречается в A.
...
def solution(N):
    return max([len(i) for i in bin(N)[2:].split('1')[:-1]])
